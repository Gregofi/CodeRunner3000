...
export async function GET({ params, request, getClientAddress }: RequestEvent): Promise<Response> {
	if (!backendUrl) {
	...
	const xforwardedfor = request.headers.get('x-forwarded-for') || getClientAddress();
